...
function assertBuildEnv(mode: string) {
  ...
  throw new Error(
    `\n\n[@collection/dashboard] Build-time environment check failed.\n\n${problems}\n\n` +
      ...
      `    local            apps/dashboard/.env        (copy apps/dashboard/.env.example)\n` +
      `    Cloudflare       Workers & Pages -> collection-dashboard -> Settings -> Build\n` +
