...
import {
  Plus, User, Car as CarIcon, Phone, MoreHorizontal,
  ChevronDown, ChevronRight, RotateCw, X, Archive, Search, Flame, Target, Boxes,
} from "lucide-react";
...
import { CarName, SpecChips, enquirySpecs, buyerSpecs } from "../matching/CarSpec";
import { channelLabel } from "../matching/channel";
import type { Car } from "@collection/shared";
import {
  isEnquiryActive, matchEnquiry, matchSellingEnquiry,
  type Enquiry, type EnquiryInput, type EnquiryType, type EnquiryMatch, type SellingMatch,
...
type Filter = "all" | "exact" | "expiring" | "today" | "sellers";
...
interface SellerDeal { seller: Enquiry; buyers: SellingMatch[]; exact: number }

export function Matching({
  enquiries, cars, loading, error, onReload, onCreate, onUpdate, onStatus, onRenew, onDelete, onOpenCar,
  getBuyingMatches = matchEnquiry, getSellingMatches = matchSellingEnquiry,
}: Props) {
  const [filter, setFilter] = useState<Filter>("all");
  const [q, setQ] = useState("");
  const [watchOpen, setWatchOpen] = useState(false);
  const [form, setForm] = useState<{ open: boolean; type: EnquiryType; editing: Enquiry | null }>({ open: false, type: "buying", editing: null });
  const [detailId, setDetailId] = useState<string | null>(null);

  const [buyMatches, setBuyMatches] = useState<Record<string, EnquiryMatch[]>>({});
  const [sellMatches, setSellMatches] = useState<Record<string, SellingMatch[]>>({});
  ...
  useEffect(() => {
    let alive = true;
    const buy = active.filter((e) => e.type === "buying").map((e) => e.id);
    const sell = active.filter((e) => e.type === "selling").map((e) => e.id);
    ...
    Promise.all(sell.map((id) => getSellingMatches(id).then((r) => [id, r] as const).catch(() => [id, null] as const)))
      .then((es) => { if (alive) setSellMatches((p) => mergeArr(p, es)); });
    return () => { alive = false; };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [enquiries, cars]);

  // --- derive the ledger -----------------------------------------------------
  const buyers = useMemo(() => active.filter((e) => e.type === "buying"), [active]);
  const sellers = useMemo(() => active.filter((e) => e.type === "selling"), [active]);
  ...
  const sellerDeals: SellerDeal[] = useMemo(() => {
    const mapped = sellers.map((s) => {
      const m = sellMatches[s.id] ?? [];
      return { seller: s, buyers: m, exact: m.filter((x) => x.tier === "exact").length };
    });
    return mapped.sort((a, b) => (b.exact - a.exact) || (b.buyers.length - a.buyers.length) || (daysLeft(a.seller.expiresAt) - daysLeft(b.seller.expiresAt)));
  }, [sellers, sellMatches]);

  const watchingBuyers = useMemo(() => buyers.filter((b) => buyMatches[b.id] && buyMatches[b.id].length === 0), [buyers, buyMatches]);
  const watchingSellers = useMemo(() => sellers.filter((s) => sellMatches[s.id] && sellMatches[s.id].length === 0), [sellers, sellMatches]);
  ...
  const strip = useMemo(() => {
    ...
    const watching = watchingBuyers.length + watchingSellers.length;
    ...
    // slow stock = actively-offered supply with no buyer
    const slow = watchingSellers[0];
    return { exactBuyers, possibleOnlyBuyers, watching, expiring, hot, sourcing, slow };
  }, [deals, watchingBuyers, watchingSellers, buyers]);
  ...
  const visibleSellerDeals = useMemo(() => sellerDeals.filter((d) => matchesQ(d.seller)), [sellerDeals, s]);

  const detail = detailId ? enquiries.find((e) => e.id === detailId) ?? null : null;
  const openNew = (type: EnquiryType) => setForm({ open: true, type, editing: null });
  ...
  const FILTERS: { key: Filter; label: string }[] = [
    ...
    { key: "today", label: "Today" }, { key: "sellers", label: "Sellers" },
  ];
  ...
  const dealActions = (buyer: Enquiry) => (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        ...
        <DropdownMenuItem onSelect={() => setForm({ open: true, type: buyer.type, editing: buyer })}>Edit</DropdownMenuItem>
    ...
    <div className="px-4 md:px-8 py-8 max-w-5xl mx-auto w-full min-w-0">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-5">
        ...
        <div className="flex gap-2 shrink-0">
          <Button variant="outline" size="sm" onClick={() => openNew("selling")}><Plus size={14} className="mr-1" />Selling</Button>
          <Button size="sm" className="bg-noir text-white hover:bg-noir-700" onClick={() => openNew("buying")}><Plus size={14} className="mr-1" />Buying</Button>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-card-border bg-white shadow-[var(--shadow-card)] px-4 py-3 mb-5 flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="hidden lg:flex items-center gap-2">
          ...
          {strip.slow && <Callout icon={<Boxes size={11} />} tone="ink" text={`Slow · ${strip.slow.make} ${strip.slow.model} offered, no buyer`} />}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <SellerLedger deals={visibleSellerDeals} onOpen={setDetailId} actions={dealActions} />
      ) : (
        <>
          ...
          {/* Watching drawer — visible, out of the actionable flow */}
          {(watchingBuyers.length + watchingSellers.length) > 0 && (
            <div className="mt-4 rounded-lg border border-border bg-alabaster/50 overflow-hidden">
              <button onClick={() => setWatchOpen((o) => !o)} className="w-full flex items-center gap-2 px-4 py-3 text-left hover:bg-platinum-soft/30 transition-colors">
                ...
                <span className="text-ink-40" style={{ fontSize: "0.78rem" }}>{watchingBuyers.length} demand · {watchingSellers.length} supply, no pairing yet</span>
              </button>
              {watchOpen && (
                <div className="divide-y divide-border/60 border-t border-border">
                  {watchingBuyers.map((b) => <WatchRow key={b.id} e={b} kind="demand" onOpen={() => setDetailId(b.id)} />)}
                  {watchingSellers.map((s2) => <WatchRow key={s2.id} e={s2} kind="supply" onOpen={() => setDetailId(s2.id)} />)}
                </div>
              )}
            </div>
          )}
      ...
      <EnquiryForm open={form.open} type={form.type} editing={form.editing} onClose={() => setForm((f) => ({ ...f, open: false, editing: null }))} onSubmit={submitForm} />
      <EnquiryDetailSheet enquiry={detail} onClose={() => setDetailId(null)}
        onEdit={(e) => { setDetailId(null); setForm({ open: true, type: e.type, editing: e }); }}
        onStatus={onStatus} onRenew={onRenew} onDelete={(id) => { onDelete(id); setDetailId(null); }} onOpenCar={onOpenCar}
        getBuyingMatches={getBuyingMatches} getSellingMatches={getSellingMatches} />
    </div>
  );
}
...
function TierChip({ tier }: { tier: "exact" | "possible" }) {
  return tier === "exact"
    ? <span className="inline-flex items-center gap-1 rounded-full bg-accent/20 text-noir px-2 py-0.5" style={{ fontSize: "0.62rem", letterSpacing: "0.08em", textTransform: "uppercase", fontWeight: 600 }}><span className="size-1.5 rounded-full bg-accent" />Exact</span>
    : <span className="inline-flex items-center rounded-full bg-platinum-soft text-ink-60 px-2 py-0.5" style={{ fontSize: "0.62rem", letterSpacing: "0.08em", textTransform: "uppercase", fontWeight: 600 }}>Possible</span>;
}
...
// Seller-anchored deal for the "Sellers" lens (reverse matching).
function SellerLedger({ deals, onOpen, actions }: { deals: SellerDeal[]; onOpen: (id: string) => void; actions: (e: Enquiry) => ReactNode }) {
  if (deals.length === 0) return <div className="rounded-lg border border-dashed border-border p-12 text-center"><div className="accent-rule mx-auto mb-4" /><p className="text-ink-60">No active selling inquiries.</p></div>;
  return (
    <div className="rounded-lg border border-card-border bg-white shadow-[var(--shadow-card)] divide-y divide-border/60">
      {deals.map(({ seller, buyers, exact }) => (
        <div key={seller.id} className="p-4 md:px-5">
          <div className="flex items-center justify-between gap-3 mb-1">
            <div className="flex items-center gap-2 min-w-0">
              <span className="shrink-0 flex size-6 items-center justify-center rounded-full bg-accent/20 text-noir"><CarIcon size={12} /></span>
              <button onClick={() => onOpen(seller.id)} className="editorial text-noir truncate hover:text-accent transition-colors text-left" style={{ fontSize: "1.05rem" }}>{seller.year ? `${seller.year} ` : ""}{seller.make} {seller.model}</button>
              {seller.variant && <span className="text-ink-40 truncate hidden sm:inline" style={{ fontSize: "0.78rem" }}>{seller.variant}</span>}
            </div>
            <div className="flex items-center gap-2.5 shrink-0"><ExpiryTag e={seller} />{actions(seller)}</div>
          </div>
          <div className="mb-2.5 flex flex-wrap items-center gap-x-2 gap-y-1">
            <span className="text-ink-40 shrink-0" style={{ fontSize: "0.72rem" }}>{seller.customerName} offering</span>
            <SpecChips specs={enquirySpecs(seller)} />
          </div>
          {buyers.length === 0 ? (
            <div className="text-ink-40 rounded-lg bg-alabaster/70 px-3 py-2.5" style={{ fontSize: "0.8rem" }}>No buyers waiting yet.</div>
          ) : (
            <div className="space-y-1.5">
              <div style={{ fontSize: "0.72rem" }} className="text-ink-60"><span className="text-noir" style={{ fontWeight: 600 }}>{buyers.length}</span> {buyers.length === 1 ? "buyer" : "buyers"} waiting{exact > 0 ? ` · ${exact} exact` : ""}</div>
              {buyers.map((b) => (
                <div key={b.refId} className={`rounded-lg px-3 py-2 ${b.tier === "exact" ? "bg-accent/[0.06]" : "bg-alabaster/70"}`}>
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2 min-w-0">
                      <TierChip tier={b.tier} />
                      <span className="text-noir truncate" style={{ fontSize: "0.85rem", fontWeight: 500 }}>{b.customerName}</span>
                    </div>
                    <a href={contactHref(b.customerPhone, b.channel)} className="shrink-0 inline-flex items-center gap-1 rounded-md border border-border px-2.5 py-1.5 text-noir hover:border-accent/40 transition-colors" style={{ fontSize: "0.76rem" }}><Phone size={12} />Contact</a>
                  </div>
                  <div className="mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-1">
                    <span className="text-ink-40 shrink-0" style={{ fontSize: "0.72rem" }}>wants</span>
                    <CarName make={b.make} model={b.model} variant={b.variant} />
                    <SpecChips specs={buyerSpecs(b)} />
                  </div>
                </div>
              ))}
            </div>
...
function WatchRow({ e, kind, onOpen }: { e: Enquiry; kind: "demand" | "supply"; onOpen: () => void }) {
  return (
    <button onClick={onOpen} className="w-full text-left px-4 py-2.5 hover:bg-platinum-soft/40 transition-colors flex items-center gap-3">
      <span className={`shrink-0 flex size-7 items-center justify-center rounded-full ${kind === "demand" ? "bg-noir/80 text-cream" : "bg-accent/15 text-noir"}`}>{kind === "demand" ? <User size={13} /> : <CarIcon size={13} />}</span>
      <span className="flex-1 min-w-0">
        ...
        <span className="text-ink-40 truncate block" style={{ fontSize: "0.74rem" }}>{e.customerName} · {kind === "demand" ? "wants — none available" : "offering — no buyer"}</span>
      </span>
      <ExpiryTag e={e} />
    </button>
  );
